    ...
    match args.command {
        Commands::Generate {
                ...
                if let Some(file_name) = file_path.file_name() {
                    if let Some(file_name_str) = file_name.to_str() {
                        if file_name_str.ends_with(".rs") {
                            ...
                            println!("Loading input file: {}", file_name_str);
            ...
            if let Some(exclude_file) = exclude_file {
                #[cfg(debug_assertions)]
                println!("Loading exclusion types from {}", &exclude_file);
                match parse_exclusion_file(&exclude_file) {
                    Ok(parsed_list) => {
                        #[cfg(debug_assertions)]
                        println!("Excluding types {:?}", parsed_list);
